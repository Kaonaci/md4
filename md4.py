...
def md4(message: str):
    ...
    # Подготовка строки
    data = bytearray([0]*(length + 64 - (length % 64))) # Формируем буфер нужного размера
    data[:length] = message # Записываем в него байты строки
    ...
    for ofs in range(0, len(data), 64):
        ...
        for i in range(16):
            ...
            A = left_rt(((A + F(B, C, D) + words[i]) & MASK), s1[i%4]);
        ...
        for i in range(16):
            ...
            A = left_rt(((A + G(B, C, D) + words[order2[i]] + const_r2) & MASK), s2[i%4]);
            A, B, C, D = D, A, B, C; # Циклически сдвигаем (A,B,C,D) -> (D,A,B,C)
        ...
        for i in range(16):
            ...
            A = left_rt(((A + H(B, C, D) + words[order3[i]] + const_r3) & MASK), s3[i%4]);
            A, B, C, D = D, A, B, C; # Циклически сдвигаем (A,B,C,D) -> (D,A,B,C)
